...
# Define ghtgbt_grid_serchthresholds to test
#thresholds = np.linspace(0,1 , num=1000)
# Loop over thresholds
results = []
#print(len(thresholds))
#print(thresholds[1500])
for thresh in range(1313,1599,1):
    ...
    fns = []
    # Loop over folds
    for train_index, test_index in kf.split(features_ensemble):
        X_train, X_test = features_ensemble[train_index], features_ensemble[test_index]
    ...
    print('mean_tn:',mean_tn)
    print('mean_fp:',mean_fp)
    print('mean_fn:',mean_fn)
   
    # Append to results list
    results.append((thresh, mean_accuracy, mean_auroc, mean_mcc,mean_sp,mean_sn,mean_tp,mean_tn,mean_fp,mean_fn))
    if thresholds[thresh]==0 :
         break
# Print results
print("Threshold\tAccuracy\tAUROC\tMCC\tSP\tSN\ttp\ttn\tfp\tfn")
for thresh, mean_accuracy, mean_auroc, mean_mcc,mean_sp,mean_sn,mean_tp,mean_tn,mean_fp,mean_fn in results:
    print("{}\t\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}\t{:.5f}".format(thresh, mean_accuracy, mean_auroc, mean_mcc,mean_sp,mean_sn,mean_tp,mean_tn,mean_fp,mean_fn))
